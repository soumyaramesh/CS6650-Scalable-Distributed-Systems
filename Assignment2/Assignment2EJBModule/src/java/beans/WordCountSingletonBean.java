...
@Singleton
public class WordCountSingletonBean implements WordCountSingletonBeanRemote {
    ...
    public void updateWordCount(String message) {
        ...
        for(String word:wordList) {
            if(!stopWordsList.contains(word)) {
                String queryString = "SELECT we FROM WordCountEntity we "
                        + "WHERE we.word = ?1";
                ...
                if(result.isEmpty()|| result.size()<1) {
                    ...
                    persist(newWordCountEntity);
                }
                else {
                    System.out.println("Result list size is " + result.size());
                    WordCountEntity we = (WordCountEntity) result.get(0);
